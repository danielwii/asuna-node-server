    ...
    const callPromise = () =>
      enabledRedis
        ? RedisLockProvider.instance
            ...
            .catch((reason) => this.logger.error(`do lock process error: ${r(reason)}`, reason.stack))
        : handler().catch((reason) => this.logger.error(`call handler ${operation} error: ${r(reason)}`, reason.stack));
